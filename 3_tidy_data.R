...
decisions <- dec_base %>% 
  mutate(decision_maker = case_when(dh02a_1 == "A" & dh02b_1 == "" ~ "Own",
                                    ...
                                    dh02_8_1 == 8 ~ "Doesn't know/answer",
                                    TRUE ~ "Other")) %>%
...
aux_adul <- basic_ind %>% 
  ...
  filter(ls02_2>18) %>%
  left_join(ed_base) %>% 
  # esta línea la agrego para ver el tema de time use - IDB
  left_join(time_base) %>% 
  filter(!ls13_2 %in% out) %>% 
  left_join(bio_base) %>% 
  left_join(cog_adul_proc) %>% 
  ...
  left_join(decisions %>% 
              select(year, folio, ls, pid_link, 
                     pid_link_uni, folio_uni, `1`: `12`),  by = c("year", "folio", "ls", "pid_link", "pid_link_uni", "folio_uni")) %>% 
  rename(cook = ata01d, clean_wash = ata01e, 
         care = ata01f, help_study = ata01g, 
         cook_hours =  ata02d, clean_wash_hours = ata02e,
         care_hours = ata02f, help_study_hours = ata02g) %>% 
  select(year, ent, state_name, mpio, dummy_div, date_int, folio, pid_link, folio_uni, pid_link_uni,  sex, ls05_1, 
         ...
         income_c, ls14, sa07_21, sa08_21, sa16_21, point, test_score, decision_points, decision_alone, HH, money_rela, 
         decision_finan, cook, clean_wash, care, help_study, cook_hours, clean_wash_hours, care_hours, help_study_hours, 
         `1`: `12`) 

#### aux adul por decisions 
