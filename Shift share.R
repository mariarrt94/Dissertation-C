...
sec_2004 <- read.csv("Inputs/SAIC__2004_sec.csv", skip = 4) %>% 
  select(-c("X", "UE.UNIDADES.ECONÓMICAS")) %>% filter(!is.na(H001A.Personal.ocupado.total))%>% 
...
sec_tot_tidy <- sec_sep %>% 
  rename(total = H001A.Personal.ocupado.total , female = H001C.Personal.ocupado.total..mujeres, 
         male = H001B.Personal.ocupado.total..hombres, sec = Actividad.Económica) %>% 
...
write.csv(shock_base, "Outputs/shocks_base_sec.csv")
