...
#### test fixed effects new ----

# regressions children new 

# works well girls 
test_new1 <- plm(test_score_z ~ PC1ch_new_mom + PC1financial_new_mom +ls02_2 + c(ls02_2*ls02_2) + school_att + edn09 + worked_12_mom + HH_mom + test_score_mom 
                   + test_score_dad  + edu_mom + edu_dad +  log_income_crea_pc + children,
                   data = pdata_child_t, model = "within", effect = "time", subset = sex == 0)

test_new2 <- plm(test_score_z ~ PC1ch_new_mom + PC1financial_new_mom +ls02_2 + c(ls02_2*ls02_2) + school_att + edn09 + worked_12_mom + HH_mom + test_score_mom 
                   + test_score_dad  + edu_mom + edu_dad +  log_income_crea_pc + children,
                   data = pdata_child_t, model = "within", effect = "individual", subset = sex == 0)

test_new1_boys <- plm(test_score_z ~ PC1ch_new_mom + PC1financial_new_mom +ls02_2 + c(ls02_2*ls02_2) + school_att + edn09 + worked_12_mom + HH_mom + test_score_mom 
                 + test_score_dad  + edu_mom + edu_dad +  log_income_crea_pc + children,
                 data = pdata_child_t, model = "within", effect = "time", subset = sex == 1)

test_new2_boys <- plm(test_score_z ~ PC1ch_new_mom + PC1financial_new_mom +ls02_2 + c(ls02_2*ls02_2) + school_att + edn09 + worked_12_mom + HH_mom + test_score_mom 
                 + test_score_dad  + edu_mom + edu_dad +  log_income_crea_pc + children,
                 data = pdata_child_t, model = "within", effect = "individual", subset = sex == 1)
...
rob_se_test_new <- list(sqrt(diag(vcovHC(test_new1, type = "HC1", cluster = "group"))),
                  #sqrt(diag(vcovHC(test_new2, type = "HC1", cluster = "group"))),
                  sqrt(diag(vcovHC(test_new1_boys, type = "HC1", cluster = "group")))
                 # sqrt(diag(vcovHC(test_new2_boys, type = "HC1", cluster = "group")))
                  )

stargazer(test_new1, test_new1_boys, 
          digits = 3, header = FALSE, type = "html", dep.var.labels=c("Raven Test Z-Score"),
          covariate.labels=c("Bargaining Index C", "Bargaining Index F", "Age", "Age squared" ,"School attendance", 
                             "Education level", "Mom works?", "Mom is household head", 
                             "Test score mom", "Test score dad", "Mom education", "Dad education",
          ...
          no.space = FALSE, align = TRUE,
          single.row = TRUE,
          se = rob_se_test_new, model.numbers = FALSE,  column.labels = c("FE girls (1)", "FE boys (2)"), 
        #  add.lines = list(c('Individual Fixed Effects', 'No', 'Yes', 'No', 'Yes')),
       #   add.lines = list(c('Time Fixed Effects', 'Yes', 'No', 'Yes', 'No')),
          column.sep.width = "5pt", # Well... you can tweak this,
          omit.stat = c("adj.rsq"),
          out = "Outputs/test_models_bg_new.htm")
...
# nre models
hfa_new_girls <- plm(height_z ~ PC1ch_new_mom + PC1financial_new_mom + ls02_2 + c(ls02_2*ls02_2) + edn09  + worked_12_mom + HH_mom + height_mom 
                  + height_dad  + edu_mom + edu_dad +  log_income_crea_pc + children,
                  data = pdata_child_h, model = "within", effect = "time", subset = sex == 0)

hfa_new_boys <- plm(height_z ~ PC1ch_new_mom + PC1financial_new_mom + ls02_2 + c(ls02_2*ls02_2) + edn09  + worked_12_mom + HH_mom + height_mom 
                  + height_dad  + edu_mom + edu_dad +  log_income_crea_pc + children,
                  data = pdata_child_h, model = "within", effect = "time", subset = sex == 1)


rob_se_bg_h <- list(sqrt(diag(vcovHC(hfa_new_girls, type = "HC1", cluster = "group"))),
                    sqrt(diag(vcovHC(hfa_new_boys, type = "HC1", cluster = "group"))))

stargazer(hfa_new_girls, hfa_new_boys,
          digits = 3, header = FALSE, type = "html", dep.var.labels=c("Height Z-score"),
          covariate.labels=c("Bargaining Index C", "Bargaining Index F", "Age", "Age squared",
                             "Education level" ,"Mom works?", "Mom is household head", 
                             "Height mom", "Height dad", "Mom education", "Dad education",
          ...
          single.row = TRUE,
          no.space = FALSE, align = TRUE,
          se = rob_se_bg_h, model.numbers = FALSE,  column.labels = c("FE girls (1)", "FE boys (2)"), 
          add.lines = list(c('Time Fixed Effects', 'Yes', 'Yes')),
          column.sep.width = "5pt", # Well... you can tweak this,
          #omit.stat = c("adj.rsq"),
          out = "Outputs/height_models_GB_new.htm")
