...
mom_base <- aux_adul %>% 
  ...
  rename(age_mom = ls02_2, spanish_mom = spanish, indigenous_mom = indigenous, worked_12_mom = worked_12, 
         ...
         PC1ch_new_mom = PC1ch_new, PC1financial_new_mom = PC1financial_new, decision_finan_mom = decision_finan) %>% 
  select(year, ent, mpio, dummy_div, folio_uni, pid_link_mom, age_mom, spanish_mom, indigenous_mom, worked_12_mom, edu_mom, married_mom, income_c_mom, 
         ...
         PC1ch_mom, decision_finan_mom, money_rela, PC1financial_new_mom, PC1ch_new_mom, `1`: `12`) %>% 
  distinct()
...
dad_base <- aux_adul %>% 
  ...
  rename(age_dad = ls02_2, spanish_dad = spanish, indigenous_dad = indigenous, worked_12_dad = worked_12, 
         ...
         PC1ch_new_dad= PC1ch_new, PC1financial_new_dad = PC1financial_new, decision_finan_dad = decision_finan) %>% 
  select(year, folio_uni, pid_link_dad, age_dad, spanish_dad, indigenous_dad, worked_12_dad, edu_dad, married_dad, income_c_dad, 
         height_dad, weight_dad, hemo_dad, test_score_dad, decision_dad,rel_hh_dad, HH_dad, PC1tot_dad, PC2tot_dad, PC1money_dad, 
         PC1ch_dad, PC1financial_new_dad, PC1ch_new_dad, decision_finan_dad) %>% 
  distinct()
...
#  filter(age_mom >=18 & age_mom <=64) %>% 
  ...
  mutate(income_c_mom = ifelse(worked_12_mom ==0 & is.na(income_c_mom), 0, income_c_mom), 
         income_c_dad = ifelse(income_c_dad ==0 & is.na(income_c_dad), 0, income_c_dad),
         income_crea = case_when(income_com != 0 ~ income_com, 
  ...
  select(year, ent, mpio, dummy_div, folio_uni, pid_link_uni, sex, ls02_2, spanish, school_att, worked_12, edn09, 
         ...
         PC1ch_mom, decision_finan_mom, decision_finan_dad, money_rela, age_dad, spanish_dad, worked_12_dad, edu_dad, height_dad, hemo_dad, test_score_dad, income_c_dad,
         decision_dad, hfa_z, test_score_z, income_crea, log_income_crea, income_crea_pc, log_income_crea_pc, children, number_persons, PC1ch_new_mom, PC1ch_new_dad, 
         PC1financial_new_mom, PC1financial_new_dad)


# in progress in case it is useful to measure relative bargaining power

dup_relation <- aux_adul %>% 
  rename(age =ls02_2, relation = ls05_1) %>% 
  filter(relation ==1 | relation ==2) %>% 
  left_join(women_index_res %>% 
              mutate(sex = ifelse(sex==3,0,sex)), by = c("year", "folio", "folio_uni", "pid_link_uni", "sex")) %>% 
  select(folio_uni, pid_link_uni, sex, relation, PC1money, PC1ch_new, year, income_c, ed06, test_score, age) %>% 
  dplyr::group_by(folio_uni, year, relation) %>%
...
dup_sex <- aux_adul %>% 
  ...
  left_join(women_index_res %>% 
              mutate(sex = ifelse(sex==3,0,sex)), by = c("year", "folio", "folio_uni", "pid_link_uni", "sex")) %>% 
  select(folio_uni, pid_link_uni, sex, relation, PC1money, PC1ch_new, year, income_c, ed06, test_score, age) %>% 
  dplyr::group_by(folio_uni, year, sex) %>%
  dplyr::summarise(n = dplyr::n(), .groups = "drop") %>%
  dplyr::filter(n > 1L) 


relative_base <- aux_adul %>% 
  rename(age =ls02_2, relation = ls05_1) %>% 
  filter(relation ==1 | relation ==2) %>% 
  left_join(women_index_res %>% 
              mutate(sex = ifelse(sex==3,0,sex)), by = c("year", "folio", "folio_uni", "pid_link_uni", "sex")) %>% 
  select(folio_uni, pid_link_uni, sex, relation, PC1money, PC1ch_new, year, income_c, ed06, test_score, age, relation) %>% 
  arrange(folio_uni, pid_link_uni, sex, year) %>% 
  filter(!folio_uni %in% dup_sex$folio_uni) %>% 
  filter(!is.na(PC1money))%>% 
  select(-pid_link_uni) %>% 
  distinct() %>% 
  pivot_wider(names_from = sex, values_from = c(PC1ch_new, PC1money, income_c, ed06, test_score, age)) 


same_sex_base <- aux_adul %>% 
  rename(age =ls02_2, relation = ls05_1) %>% 
  filter(relation ==1 | relation ==2) %>% 
  left_join(women_index_res %>% 
              mutate(sex = ifelse(sex==3,0,sex)), by = c("year", "folio", "folio_uni", "pid_link_uni", "sex")) %>% 
  select(folio_uni, pid_link_uni, sex, relation, PC1money, PC1ch_new, year, income_c, ed06, test_score, age, relation) %>% 
  arrange(folio_uni, pid_link_uni, sex, year) %>% 
  filter(!folio_uni %in% dup_relation$folio_uni) %>% 
  filter(!is.na(PC1money))%>% 
  select(-pid_link_uni) %>% 
  distinct() %>% 
  pivot_wider(names_from = relation, values_from = c(PC1ch_new, PC1money, income_c, ed06, test_score, sex, age)) %>% 
  mutate(same_sex = ifelse(sex_1 == sex_2, 1, 0)) %>% 
  filter(same_sex ==1)

# remove same sex 
mom_base <- mom_base %>% 
  filter(!(folio_uni %in% same_sex_base$folio_uni))
